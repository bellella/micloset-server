  ...
  async validate(req: Request): Promise<any> {
    ...
    if (!accessToken) {
      throw new UnauthorizedException(
        'Google Access Token not found in the request body.'
      );
    }

    try {
      // Request user info from Google's endpoint using the Access Token
      ...
      // Validate essential fields (security check)
      if (!data.email || !data.email_verified) {
        throw new UnauthorizedException('Email is not verified or missing.');
      }

      // Create a structure similar to the ID Token payload for upsert
      const userProfile = {
        email: data.email,
